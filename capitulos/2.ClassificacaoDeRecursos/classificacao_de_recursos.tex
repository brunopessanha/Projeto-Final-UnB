...
\begin{itemize}
	...
	\item IDL(\emph{Interface Description/Definition Language}): linguagem que suporta especificação e descrição de interfaces de um objeto independente da linguagem de programação utilizada.

	\item Arquivo Descritor: arquivo estruturado, geralmente binário, onde cada \emph{byte} ou \emph{bit} representa uma informação. Essa informação pode representar a tipagem do dado, o valor do dado ou um estado, dependendo da sua definição.
