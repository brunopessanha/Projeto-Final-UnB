...
O fator de movimentação não foi implementado na aplicação para Android, pois foi utilizado um dispositivo com tela sensível ao toque que torna mais natural a movimentação do cursor do \emph{mouse} assemelhando-se com o uso de um \emph{touchpad} de \emph{notebook}. A interação com o recurso de \emph{mouse} é feita por meia de um toque simples para \emph{clicks} e movimentação de um dedo para mover o cursor do mouse e movimentação de dois dedos na vertical para mover a barra de rolagem. Ao se utilizar a tela sensível ao toque obteve-se uma grande vantagem em relação à implementação anterior, pois é possível movimentar o cursor do \emph{mouse} simultaneamente em qualquer direção. A figura~\ref{fig:printscreen_android} mostra a tela do protótipo utilizado.	

\begin{figure}[ht]
	\center
	\includegraphics[scale=0.3]{imagens/printscreenDetectedDrivers}
	\caption{Tela inicial da \emph{Hydra}.}
	\label{fig:printscreen_hydra1}
\end{figure}

\begin{figure}[ht]
	\center
	\includegraphics[scale=0.3]{imagens/printscreenMouseDrivers}
	\caption{Tela de listagem e seleção de recursos de \emph{Mouse}.}
	\label{fig:printscreen_hydra2}
\end{figure}

As figuras~\ref{fig:printscreen_hydra1} e~\ref{fig:printscreen_hydra2} mostram os dois protótipos sendo reconhecidos pela aplicação \emph{Hydra}. Na primeira imagem, tela inicial da \emph{Hydra}, são mostrados os dispositivos \emph{SamsungGalaxyS} e \emph{NokiaN96} com seus respectivos \emph{drivers}: \emph{DeviceDriver} e \emph{MouseDriver} cujas instâncias têm nome de \emph{droid\_mouse\_instance} e \emph{defaultMouseDriver}. A segunda imagem mostra a aba da aplicação \emph{Hydra} que lista apenas os recursos de \emph{mouse} encontrados.
